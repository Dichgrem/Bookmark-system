<template>
  <div class="app">
    ...
    <div v-if="!isLogin" class="login-box">
      <h3>登录 / 注册</h3>
      <el-input v-model="loginForm.username" placeholder="用户名" class="mb10" />
      <el-input v-model="loginForm.password" placeholder="密码" type="password" class="mb10" />
      <div class="btn-row">
    ...
    <div v-else class="main">
      <div class="cate-add">
        <el-input v-model="cateName" placeholder="分类名称" style="width:240px" />
      ...
      <div class="cate-list">
        <div v-for="cate in cateList" :key="cate.id" class="cate-card">
          ...
          <div class="book-add">
            <el-input v-model="bookTitle" placeholder="书签名" style="width:140px" />
            <el-input v-model="bookUrl" placeholder="网址" style="width:240px" />
            <el-button type="success" size="small" @click="addBook(cate.id)">添加</el-button>
          </div>

          <div class="book-list">
            <div v-for="book in bookList.filter(b=>b.categoryId===cate.id)" :key="book.id" class="book-item">
...
.app { background: #f5f7fa; min-height: 100vh; }
.nav { background: #409eff; color: white; padding: 14px 30px; font-size: 18px; }
.login-box { background: white; width: 400px; margin: 80px auto; padding: 30px; border-radius: 12px; box-shadow: 0 2px 12px #00000010; }
.mb10 { margin-bottom: 10px; }
.btn-row { display: flex; gap: 10px; justify-content: center; margin-top: 10px; }
.main { max-width: 1000px; margin: 0 auto; padding: 20px; }
.cate-add { background: white; padding: 16px 20px; border-radius: 8px; margin-bottom: 20px; display: flex; gap: 10px; align-items: center; }
.cate-list { display: flex; flex-direction: column; gap: 16px; }
.cate-card { background: white; padding: 20px; border-radius: 12px; box-shadow: 0 2px 8px #0000000c; }
.cate-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
.book-add { display: flex; gap: 8px; margin-bottom: 12px; }
.book-list { display: flex; flex-direction: column; gap: 8px; }
.book-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: #f9f9f9; border-radius: 6px; }
